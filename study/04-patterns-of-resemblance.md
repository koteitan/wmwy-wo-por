...
展開（[05](05-omegay-mountain.md) §4）は、数列から新しい数列を作る操作である。数列の各項を列と呼ぶ（[05](05-omegay-mountain.md) §1）。展開の停止性の証明では、列ごとに順序数のラベル（[01](01-ordinals.md) §6）を付け、展開でラベルが下がることを示す（[02](02-well-founded.md) §6）。そこで要る性質は **有限反映** である。
...
展開では、この形を次のように使う。ある列を選び、切れ目と呼ぶ（[06](06-combinatorial-layer.md) §5）。$`\alpha`$ を切れ目の列の古いラベルとする。切れ目より前の列の古いラベルを $`\vec p`$、ラベルを付け替える列の古いラベルを $`\vec y`$ とする。$`\vec y`$ はどれも $`\alpha`$ 以上である。$`\psi`$ には、列のラベルの間の関係を書いておく。例えば、山（[05](05-omegay-mountain.md) §3）で辺で結ばれた 2 つの列（親と子）のラベルが、関係 $`\le_1`$ などを満たすことである。有限反映で得た新しいラベル $`\vec y'`$ も同じ関係を満たし、どれも $`\alpha`$ より下にある。したがって新しいラベルは古いラベルより小さい。
...
$`m`$ は鍵の長さである。鍵 $`\theta`$ は、ラベルか $`\top`$ を $`m`$ 個並べた列である（[02](02-well-founded.md) §3）。3 番目の引数 $`b`$ は上端である（[02](02-well-founded.md) §3）。このリポジトリでは $`R(\theta, a, b)`$ を「上端の述語を鍵が $`\theta`$ より下のところでだけ定義した言語（§5）で、高さ $`a`$ の構造が高さ $`b`$ の構造の $`\Sigma_1`$ 初等部分構造である」と読む（[07](07-relation-r.md)）。このため、3 つの問題が起きる。
...
**問題 2：上端への要求.** 有限反映は、点 $`x`$ から上端 $`b`$ への関係 $`R(\kappa, x, b)`$（$`\kappa`$ は鍵）も、新しいラベルで成り立たせる必要がある。この形の条件を要求と呼ぶ（[06](06-combinatorial-layer.md) §2 の上端の原子）。$`b`$ は構造 $`(b; \ldots)`$ の元ではない。$`R`$ の定義を展開して書くと $`\Sigma_1`$ にならない。
...
2. **上端の述語を原子記号にする.** 高さ $`c`$ の構造は、型板 $`t`$ と位置 $`i`$ ごとに記号 $`\mathrm{Top}_{t,i}`$（[03](03-sigma1-elementary.md) §7）を持ち、「$`R(\mathrm{eval}\ t\ \vec v, v_i, c)`$」と解釈する。上端への要求は原子式になる（問題 2）。
...
| 上端との関係 | 連続性・共終性の補題 | 原子記号（定義されるかどうかは変数の位置で決まる） | 図式の中の要求 | 部分的な原子記号（鍵の値で定義される） |
| 再帰 | 上端 $`\beta`$ | （上端、$`k`$、$`\eta`$） | （上端、鍵） | （上端、鍵） |

表の言葉は次のとおりである（[notes/00-survey.md](../notes/00-survey.md) §3.1、§3.2）。

- 1-Y 版の $`(k, \eta)`$：$`k`$ は 1-Y の山の行の番号、$`\eta`$ は根の列のラベルである。
- Phyrion 氏の図式：有限個の点の上の、点の間の関係 $`R`$ の有限個の条件と、点から上端への要求の有限個の条件の組である（[06](06-combinatorial-layer.md) §2 の図式と同じ形）。正とは、条件に否定を含まないことである。
