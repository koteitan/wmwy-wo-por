...
| [02 整礎関係と整礎再帰](02-well-founded.md) | 整礎再帰、ガードつきの再帰、鍵 $`\mathrm{Key}_m`$、段、上端 |
| [03 構造と Σ₁ 初等部分構造](03-sigma1-elementary.md) | 構造 $`(\gamma; \ldots)`$、$`\Sigma_1`$ 論理式、証人、$`\preccurlyeq_{\Sigma_1}`$、型板、内部の関係、上端の述語、部分的な上端の述語 |
...
- $`x \le_1 y`$ の真偽は、$`y`$ についての再帰の段 $`y \lt \beta`$ で決まっている。
...
展開（[05](05-omegay-mountain.md) §4）は、数列から新しい数列を作る操作である。数列の各項を **列** と呼ぶ。展開の停止性の証明では、列ごとに順序数の **ラベル** を付け、展開でラベルが下がることを示す（[02](02-well-founded.md) §6）。そこで要る性質は **有限反映** である。
...
展開では、この形を次のように使う。ある列を選び、**切れ目** と呼ぶ。$`\alpha`$ を切れ目の列の古いラベルとする。切れ目より前の列の古いラベルを $`\vec p`$、ラベルを付け替える列の古いラベルを $`\vec y`$ とする。$`\vec y`$ はどれも $`\alpha`$ 以上である。$`\psi`$ には、列のラベルの間の関係を書いておく。例えば、山（[05](05-omegay-mountain.md) §3）で辺で結ばれた 2 つの列（親と子）のラベルが、関係 $`\le_1`$ などを満たすことである。有限反映で得た新しいラベル $`\vec y'`$ も同じ関係を満たし、どれも $`\alpha`$ より下にある。したがって新しいラベルは古いラベルより小さい。
...
ω-Y の組合せの層（Phyrion 氏の証明のうち、山と展開の形だけを扱う部分。[06](06-combinatorial-layer.md)）が要求するラベルの関係は、3 つの引数を持つ。

```math
R(\theta, a, b) \quad (\theta \in \mathrm{Key}_m,\ a, b \in \mathrm{Label})
```

$`m`$ は鍵の長さである。鍵 $`\theta`$ は、ラベルか $`\top`$ を $`m`$ 個並べた列である（[02](02-well-founded.md) §3）。3 番目の引数 $`b`$ は上端である（[02](02-well-founded.md) §3）。このリポジトリでは $`R(\theta, a, b)`$ を「上端の述語を鍵が $`\theta`$ より下のところでだけ定義した言語（§5）で、高さ $`a`$ の構造が高さ $`b`$ の構造の $`\Sigma_1`$ 初等部分構造である」と読む（[07](07-relation-r.md)）。$`\mathcal R_N`$ の $`\le_j`$ の $`j`$ や、$`R`$ の第 1 引数 $`\theta`$ のように、関係の族を区別する引数を **関係の添字** と呼ぶ。この関係のため、3 つの問題が起きる。

**問題 1：関係の添字が超限である.** 鍵は $`\mathrm{Key}_m`$ を辞書式に動く。座標はラベルなので、鍵の順序は超限である。$`\mathcal R_N`$ の関係の添字 $`1, \ldots, N`$（$`\Sigma_1, \ldots, \Sigma_N`$ の初等性）は有限個で、自然数で数える。鍵をこの番号にできない。

**問題 2：上端への要求.** 有限反映は、点 $`x`$ から上端 $`b`$ への関係 $`R(\kappa, x, b)`$（$`\kappa`$ は鍵）も、新しいラベルで成り立たせる必要がある。この形の条件を **要求** と呼ぶ（[06](06-combinatorial-layer.md) §2 の上端の原子）。$`b`$ は構造 $`(b; \ldots)`$ の元ではない。$`R`$ の定義を展開して書くと $`\Sigma_1`$ にならない。

**問題 3：要求の鍵は動く点を名指す.** 列のラベルを $`f`$（$`f(i)`$ は列 $`i`$ のラベル）とする。要求は $`R(\mathrm{eval}\ t\ f, f(p), b)`$ の形である。$`p`$ は列の番号、$`t`$ は型板（[03](03-sigma1-elementary.md) §7）である。型板は、鍵の各座標にどの列のラベルを置くかを決める。これを「型板が列を名指す」と言う。Phyrion 氏の有限反映は、その列が切れ目（§3）より前にあることを要求しない（[OmegaY/Reflection.lean](../OmegaY/Reflection.lean) の `finite_reflection` の注釈「No root used in a key is required to be retained」）。つまり鍵の中のラベルは、反映で付け替わる証人（[03](03-sigma1-elementary.md) §2）でもよい。1-Y 版（[01](01-ordinals.md) §7）は、上端の述語が定義されるかどうかを「パラメータの位置」で決めた。その方法はここでは使えない。

## 5. このリポジトリの変更点

[notes/01-design.md](../notes/01-design.md) §2 のとおり、次のように変えた。

1. **どの関係の添字でも $`\Sigma_1`$ 初等性にする.** 添字による強さの違いは、量化子の複雑さではなく、どの上端の述語が定義されているかで決まる。
2. **上端の述語を原子記号にする.** 高さ $`c`$ の構造は、型板 $`t`$ と位置 $`i`$ ごとに記号 $`\mathrm{Top}_{t,i}`$（[03](03-sigma1-elementary.md) §7 の $`\mathrm{Top}_t(v_i)`$）を持ち、「$`R(\mathrm{eval}\ t\ \vec v, v_i, c)`$」と解釈する。上端への要求は原子式になる（問題 2）。
...
4. **内部の関係はすべての鍵で持つ.** 型板 $`t`$ と位置 $`i, j`$ ごとに記号 $`\mathrm{Rel}_{t,i,j}`$（[03](03-sigma1-elementary.md) §7 の $`\mathrm{Rel}_t(v_i, v_j)`$）を持ち、$`\mathrm{Rel}_{t,i,j}(\vec v) :\iff R(\mathrm{eval}\ t\ \vec v, v_i, v_j)`$ と解釈する。
...
|---|---|---|---|---|
| 関係の添字 | $`j = 1, \ldots, N`$ | $`(k, \eta) \in \mathbb N \times \mathrm{Ord}`$ | 鍵 $`\theta \in \mathrm{Key}_m`$ | 鍵 $`\theta \in \mathrm{Key}_m`$ |
...
| 再帰の段 | 上端 $`\beta`$ | （上端、層、添字） | （上端、鍵） | （上端、鍵） |

表の言葉は次のとおりである（[notes/00-survey.md](../notes/00-survey.md) §3.1、§3.2）。

- 1-Y 版の $`(k, \eta)`$：$`k`$ は 1-Y の山の行の番号で、**層** と呼ぶ。$`\eta`$ は根の列のラベルで、**添字** と呼ぶ。
- 1-Y 版の「見えるかどうか」：上端の述語が定義されるかどうかのことである。
- Phyrion 氏の **図式**：有限個の点の上の、点の間の関係 $`R`$ の有限個の条件と、点から上端への要求の有限個の条件の組である（[06](06-combinatorial-layer.md) §2）。**正** とは、条件に否定を含まないことである。
- 「$`b`$ より下の有限の正の図式を $`a`$ より下へ圧縮できる」：$`b`$ より下の狭義増加なラベル付けで、図式の点の間の関係が成り立ち、要求（鍵は $`\theta`$ より小さいもの）が上端 $`b`$ について成り立つとする。このとき、$`a`$ より下の狭義増加なラベル付けで、同じ関係が成り立ち、要求が上端 $`a`$ について成り立つものがある。もとのラベルのうち $`a`$ より下のものは動かさない。
