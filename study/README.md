...
このリポジトリを読むための背景ノート。証明が既知として使う数学（順序数、整礎再帰、モデル論）と、証明の 2 つの層を、定義と小さい例から書き起こす。どのノートも、Lean のコードが実際にしていることに合わせ、Lean の名前を挙げる。

- **組合せの層**：Phyrion 氏の ω-Y の証明のうち、山と展開の形だけを扱う部分。ラベルの関係 $`R`$ の定義を使わず、$`R`$ についての 3 つの定理だけを使う（[06](06-combinatorial-layer.md)）。
- **意味の層**：関係 $`R`$ を定義し、その 3 つの定理を示す部分。このリポジトリでは $`\Sigma_1`$ 初等部分構造の関係である（[07](07-relation-r.md)〜[09](09-obligations.md)）。
