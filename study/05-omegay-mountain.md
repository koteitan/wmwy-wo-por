...
**例（$`(1, 4, 20)`$）.** 表の「$`v \leftarrow (j, h)`$」は、値が $`v`$ で、左の脚（下の節点からの辺の父）が列 $`j`$ の行 $`h`$ の節点であることを表す。Lean の参照 `Ref` は行の代わりに下からの番号を使う。最下の行の左の脚（左の列の phantom）は書かない。ω-Y の山には 1-Y のような層は無く、全体が 1 枚の表になる。
...
- 列 1：最下の節点（値 4）の左の脚は列 0 の phantom で、そこから行 1 の節点（値 1）へ登る。$`1 \lt 4`$ なので父である。次の節点は行 $`B(1, 1) = 2`$、値 3。行 2 の節点の左の脚は $`(0, 1)`$ で、列 0 にはその上の節点が無いので、父はまた $`(0, 1)`$ である。行 $`B(2, 1) = 2 + \omega = \omega`$（跳び 1）、値 2。同じく行 $`B(\omega, 1) = \omega + \omega^2 = \omega^2`$（跳び 2）、値 1 で終わる。
- 列 2：どの節点でも、Q の手順の最初の候補がそのまま父になる。次の表は、下の節点から順に、父の探し方と次の節点の行と値を書いたものである。

| 節点（行、値） | 候補（Q の手順） | 父 | 次の節点の行 $`B`$ | 次の値 |
|---|---|---|---|---|
...
列 2 の父は、列 1 の節点を下から順にたどる。父が変わった最初の節点は父と同じ行にあり、跳びは 0 である。その後、同じ父の間は跳びが 1、2 と大きくなる。新しい行が列 1 の次の節点の行に届くと（行 $`\omega`$ と $`\omega^2`$）、Q の手順でその節点へ登り、父が変わる。値は、父の値（列 1 の値 4、3、2、1）を引いて下がっていく。
...
- **充填**（`fill`、weak magma）：$`\mu`$ から上への源の辺の父を $`p`$ とする。列 $`\mathrm{col}(p) + b w`$ の節点 $`q`$ で $`\mathrm{row}(\mu) \le \mathrm{row}(q) \lt g`$ のものすべてについて、$`q`$ の真上の節点を $`q^+`$ とし、行 $`\mathrm{row}(q) + \omega^i`$（$`i = \mathrm{jump}(\mathrm{row}(q), \mathrm{row}(q^+)) - 1, \ldots, 0`$）に節点を置く。左の脚はどれも $`q`$ である。充填で置いた節点を **すき間の節点** と呼ぶ。
