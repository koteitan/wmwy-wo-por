...
|---|---|
| [01 Ordinals and ω₁](01-ordinals.md) | ordinal, Cantor normal form of ordinals below $`\omega^\omega`$ |
...
- The standard seeds are $`(1, m)`$ with $`m \ge 2`$. In Lean `Dynamics.seed n` $`= (1, n+2)`$.
- Expressions are ordered lexicographically (`Dynamics.Lex`). A proper prefix is smaller. As in [02](02-well-founded.md) §1, this order is not well-founded on all expressions.
...
The order compares at the largest exponent where the coefficients differ (`Row.LexLt`). This is the order of ordinals. Lean displays a row (`Row.toList`) as its coefficients in increasing order of exponent.
...
If $`a = b`$, the row goes up by 1. This is how rows of a 1-Y mountain move. If $`a \ne b`$, the row jumps by a power of $`\omega`$.

## 3. Building the mountain

A mountain is an array of columns, and a column is an array of nodes from bottom to top (`Canonical.Mountain`, `Canonical.Column`). A node `Cell` has three components: the row `row`, the value `value`, and the left leg `left`, which points to a node of a column to the left.
...
**Definition (finding the father, `findParent`).** Start with the candidate $`w`$ equal to $`u`$ itself. Repeat the following Q step (`nextCandidate`).

- Follow the left leg $`L`$ of $`w`$. In the column of $`L`$, climb up from $`L`$ as long as the next node's row is at most $`\mathrm{row}(w)`$ (`climb`). The node reached is the new candidate $`w`$.
- If the new candidate has $`0 \lt \mathrm{value}(w) \lt v`$, then $`w`$ is the father. Otherwise repeat the Q step from this $`w`$.

In the bottom row the candidates are the bottom nodes of columns $`c-1, c-2, \ldots`$ in turn. So the father of a bottom node is the bottom node of the rightmost column with value smaller than $`s_c`$. This is the same as the parent in row 0 of 1-Y.
...
**Definition (expansion `expand s N`).** $`N`$ is the number of copies.
...
3. **Boundary rows.** List, from the top, the row of the top node of column $`x`$ and the rows of the nodes of the root column at or below the root, excluding the phantom (`boundaries`).
...
5. **Blocks.** For $`b = 1, \ldots, N`$ in order, copy the columns $`y = c_r + 1, \ldots, x`$ to the columns $`y + b w`$ (`copyBlock`, `copyColumn`).
...
**Copying a column (`copyColumn`).** When column $`y`$ is copied in block $`b`$, three kinds of nodes are placed for each marker $`\mu`$ of $`y`$.

- **Translation** (`copyEdge`): a node in row $`\mathrm{row}(\mu)`$. Its left leg is the same node as the left leg of $`\mu`$ if that is left of the root column; otherwise it is the highest node with row below $`\mathrm{row}(\mu)`$ in the column "that column $`+ b w`$". A phantom marker is copied to a phantom.
- **Contour** (`contour`): first fix the reference row $`g`$. In the current last column (the copy of column $`x`$ in block $`b-1`$; for $`b = 1`$, column $`x`$ of $`M(s')`$), take for each boundary row the highest node strictly below it (`below`). $`g`$ is the row of the last of these whose row is at least $`\mathrm{row}(\mu)`$ (`referenceAt`). Then copy the source edges upward from $`\mu`$ in order. Stop before an edge whose upper end is another marker. Stop at the top of the column. A source edge of degree $`d`$ becomes an edge from the current row $`t`$ to the row $`t + \omega^d`$; the first $`t`$ is $`g`$. The left leg is the image of the father of the source edge, by the same rule as for translation.
- **Fill** (`fill`, the weak magma): let $`p`$ be the father of the source edge from $`\mu`$ upward. For every node $`q`$ of column $`\mathrm{col}(p) + b w`$ with $`\mathrm{row}(\mu) \le \mathrm{row}(q) \lt g`$, let $`q^+`$ be the node directly above $`q`$, and place nodes in rows $`\mathrm{row}(q) + \omega^j`$ for $`j = \mathrm{jump}(\mathrm{row}(q), \mathrm{row}(q^+)) - 1, \ldots, 0`$. The left leg of each is $`q`$.
...
The official ω-Y is defined by `expand` in Naruyoko's program ([notes/00-survey.md](../../notes/00-survey.md) §1.2). Weak-magma ω-Y does not agree with it: on 3001 standard forms, 480 of 9003 expansions give different results (notes/00-survey.md §1.6).
...
- official: the gap copies the horizontal connections of the row at the same level as the root. The left legs lie in the image of the column of the left leg in that row. In the bottom row this is the column immediately to the left.
...
notes/02-feasibility.md counts the bottom row as 0: it writes $`\delta`$ for the Lean row $`1 + \delta`$. For example "$`1 \leftarrow c_2@0`$" in that note is "row 2, left leg $`(2, 1)`$" in this note.
...
**On "no extraction".** Phyrion calls this variant "weak magma, no extraction". The Lean definition has no step corresponding to the extraction of layers in 1-Y; it uses a single mountain whose rows are ordinals (the table in notes/00-survey.md §3.5). The exact meaning of extraction in ω-Y has not been checked (notes/00-survey.md §1.5).
...
- If all row coefficients of the mountain above exponent $`D`$ are 0, the same holds for the mountain after expansion (`expandDiagram_key_dimension`, `Dynamics.next_key_dimension`). So one $`D`$, chosen for the starting expression, serves all its descendants (`Dynamics.fixed_dimension_for_descendants`). $`D`$ fixes the length $`D + 1`$ of the keys in [06](06-combinatorial-layer.md).
